...
// IsRstFile reports whether name looks like a Rst file based on its extension.
func IsRstFile(name string) bool {
...
// RstRenderer is a extended version of underlying Rst render object.
...
/*
var validLinksPattern_rst = regexp.MustCompile(`^[a-z][\w-]+://|^mailto:`)

// isLink reports whether link fits valid format.
func isLink_rst(link []byte) bool {
	return validLinksPattern.Match(link)
}
*/

// Link defines how formal links should be processed to produce corresponding HTML elements.
func (r *RstRenderer) Link(out *bytes.Buffer, link []byte, title []byte, content []byte) {
...
func (r *RstRenderer) AutoLink(out *bytes.Buffer, link []byte, kind int) {
...
func (options *RstRenderer) ListItem(out *bytes.Buffer, text []byte, flags int) {
...
// RawRst renders Rst to HTML without handling special links.
func RawRst(body []byte, urlPrefix string) []byte {
	htmlFlags := 0
...
//    body = []byte("FIXME just testing")

    p := rst.NewParser(nil)
    var b bytes.Buffer
    w := bufio.NewWriter(&b)
    p.ReStructuredText(bytes.NewReader(body), rst.ToHTML(w))
    w.Flush()
...
// Rst takes a string or []byte and renders to HTML in Rst syntax with special links.
func Rst(input interface{}, urlPrefix string, metas map[string]string) []byte {
	return Render(RST, input, urlPrefix, metas)
}
